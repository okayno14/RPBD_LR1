...
void Controller::testFindPhone01(bool isOnline) 
{
	...
	//инициализац€ контакта и телефона
	PhoneNumber pn(phoneNumber, type);
	Person p(lastname,firstname,fathername);

	p.addPhoneNumber(&pn);

	//вставка объектов в справочники модели, обход€ insert()
	//model->personTable.push_back(p);
	//model->phoneNumberTable.push_back(pn);

	//вставленному контакту прив€зываем указатель вставленного в справочник телефона
...
void Controller::testFindPhoneAddress01(bool isOnline) 
{
	...
	//инициализац€ контакта, телефона, адреса
	PhoneNumber pn(phoneNumber, type);
	Person p(lastname, firstname, fathername);
	...
	//вставленному контакту прив€зываем указатель вставленного в справочник телефона и адреса
	model->personTable.back().addPhoneNumber(&model->phoneNumberTable.back());
